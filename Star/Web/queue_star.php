...
<div id="wrap" class="menu_manager">
    ...
    <div class="flexsearch">
        <div class="flexsearch--wrapper">
            <form class="flexsearch--form" action="queue_star.php" method="post">
...
function show(){
    ...
    echo "<div>";
    echo "<center><h1>คิวที่รอการยืนยัน</h1></center>";
    ...
    $dbname = "id615238_case";
                ...
                $sql = "SELECT First_name_star,Last_name_star,Id_reserve,Time_work,Date_work,Detail_work,Total_time_work,The_propose_price,P_picture FROM star LEFT OUTER JOIN picture ON Id_star = P_id_star JOIN reserve ON R_id_star = Id_star and State_reserve = 1 and S_id_star_manager = '$Id_star_manager' group By Id_reserve ASC";
            ...
            ?>
                        ...
                        <div class="position">
                            <input type="button" class="button left" name="confirmList" value="ยืนยันการจอง" onclick="confirmList(<?php echo $row['Id_reserve'];?>)">
                ...
                <?php  
                    ...
                    echo "<center><h3>ไม่มีรายการรอการยืนยัน</h3></center>";
                }
                ?>
            </div>
...
function cancleList(id) {
    if(confirm("คุณต้องการยกเลิกการจองคิวใช่ไหม")) {
        window.location.href='cancle_q.php.php?id_reserve='+id;
        return true;
    }
}
function confirmList(id) {
    if(confirm("คุณต้องการยืนยันการจองคิวใช่ไหม")) {
        window.location.href='confirm_q.php?id_reserve='+id;
        return true;
    }
}
</script>
...
$dbname = "id615238_case";
  ...
  $conn->set_charset("utf8");
  $sql = "SELECT First_name_star,Last_name_star,Id_reserve,Time_work,Date_work,Detail_work,Total_time_work,The_propose_price,P_picture
        FROM star LEFT OUTER JOIN picture ON Id_star = P_id_star JOIN reserve ON R_id_star = Id_star and State_reserve = 1 and S_id_star_manager = '$Id_star_manager' AND
            (First_name_star LIKE '%$search%' || Last_name_star LIKE '%$search%')
            ...
            ?>
                        ...
                        <?php echo '<div class="image"><img src="data:image/jpeg;base64,'.base64_encode($row['P_picture'] ).'" height="500" width="500" class="img-thumnail" /> </div>'; ?>
                        <div class="position">
                            <input type="button" class="button left" name="confirmList" value="ยืนยันการจอง" onclick="confirmList2(<?php echo $row['Id_reserve'];?>)">
