...
<!-- top-brands -->
	<div class="top-brands">
		...
		<?php 
		if(isset($_GET['id'] )){
	echo "<h2>ลอตเตอรี่ทั้งหมดที่ออกรางวัลเมื่อวันที่ ".$_GET['id']."</h2>";
		}
	?>
...
if(isset($_GET['id'] )){
	...
	// echo $_POST['reports'];
 	// if($_POST['reports']=='รายงานสต็อก'){
 		$lottery = new lottery;
 		$d = $_GET['id'];
 		$data = $lottery->viewStock('bylot',$d);

		?>
		<table class="table table-hover" border = 1 align = 'center'>			
 		<tr align = 'center'> <td class="danger">หมายเลขลอตเตอรี่</td>
 			<td class="danger">งวดที่</td><td class="danger">จำนวนนำเข้า</td>
 			<td class="danger">คงเหลือ</td></tr>
...
echo "<tr align = 'center'> <td class='warning'>".$read["lottery_number"]."</td>
	<td class='warning'>".$read["period"]."</td>
 <td class='warning'>30</td><td class='warning'>".$read["quantity"]."</td></tr> ";
